...
Down quarks & 9 & $9/4$ & 4{,}278 & 4{,}278 & $\sim 0$ \\
...
\noindent
The lepton and up-quark sectors agree at the $0.01\%$ level.
The down-quark sector sum includes a cross-representation vertex
correction derived below; with this correction all four sectors
are reproduced from zero free parameters at the $0.01\%$ level
or better.

\begin{derivation}[$\Dmark$\; Cross-rep vertex correction]
\label{der:vertex_correction}
The tree-level mass formula gives $\Sdown^{(0)} = \tfrac{9}{4}\,\mP\,
e^{-(9\Reff+\delta)/28} = 4{,}237$~MeV, which is 0.96\% below
experiment.  The correction arises from the different Yukawa coupling
density in the cross-representation channel ($\mathbf{10} \times
\overline{\mathbf{5}}$) versus the same-representation channel
($\mathbf{10} \times \mathbf{10}$).

\medskip
\noindent\textbf{Step 1: Coupling density from $\Eeight$ structure constants.}
A root pair $(\alpha,\beta)$ forms a Yukawa vertex when $\alpha + \beta$
is also a root, which occurs exactly when $\langle\alpha,\beta\rangle = -1$.
We count such pairs in each channel:
\begin{center}
\small
\begin{tabular}{lccc}
\toprule
Channel & Total pairs & Pairs with $\langle\alpha,\beta\rangle = -1$ &
  Density $\rho$ \\
\midrule
Same-rep $(\mathbf{10}\cup\overline{\mathbf{10}})^2$ &
  $\binom{48}{2} = 1128$ & 272 & $272/1128$ \\[2pt]
Cross-rep $(\mathbf{10}\cup\overline{\mathbf{10}}) \times
  (\mathbf{5}\cup\overline{\mathbf{5}})$ &
  $48 \times 140 = 6720$ & 1520 & $1520/6720$ \\
\bottomrule
\end{tabular}
\end{center}
The coupling density ratio is
\begin{equation}
\frac{\rho_{\text{same}}}{\rho_{\text{cross}}}
= \frac{272 \times 6720}{1520 \times 1128}
= \frac{952}{893}.
\end{equation}

\noindent\textbf{Step 2: Exposure fraction from nearest-neighbor combinatorics.}
The $\mathbf{10}$ representation contains 20~active roots
(those with $Q = \pm 3/2$ that couple to $\overline{\mathbf{5}}$).
Each has exactly~9 nearest neighbors in $\overline{\mathbf{5}}$:
3~cross-sector (different lattice type) and 6~same-sector.
The 3~cross-sector directions correspond to $N_c = 3$ independent
color channels, giving the exposure fraction
\begin{equation}
\frac{N_c}{N_{\text{active}}} = \frac{3}{20}.
\end{equation}

\noindent\textbf{Step 3: The correction.}
The cross-rep Yukawa has 6.6\% lower coupling density than the
same-rep channel.  Since the mass formula is a product over
independent lattice channels (Boltzmann factor structure), the
correction is multiplicative:
\begin{equation}
\label{eq:vertex_correction}
\boxed{\Sdown = \Sdown^{(0)} \times
\Bigl(\frac{\rho_{\text{same}}}{\rho_{\text{cross}}}
\Bigr)^{N_c/N_{\text{active}}}
= \Sdown^{(0)} \times
\Bigl(\frac{952}{893}\Bigr)^{3/20}
\approx 1.009643 \times \Sdown^{(0)}.}
\end{equation}
Result: $\Sdown = 4{,}278$~MeV vs.\ experiment $4{,}278$~MeV
(residual error: $-0.008\%$, a $128\times$ improvement over tree level).
All ingredients --- the pair counts $272$, $1520$, the active root
count~$20$, and the cross-sector count~$3$ --- are computed from
the $\Eeight$ root system with zero free parameters.
\end{derivation}
...
$d$ & 4.681 & 4.67 & $+0.23\%$ & $\Dmark$ \\
$s$ & 93.4 & 93.4 & $+0.04\%$ & $\Dmark$ \\
$b$ & 4180 & 4180 & $-0.01\%$ & $\Dmark$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
$|V_{ud}|$ & 0.97456 & 0.97373 & $+2.7\sigma$ & $\Dmark$ \\
$|V_{us}|$ & 0.22409 & 0.22430 & $-0.41\sigma$ & $\Dmark$ \\
$|V_{ub}|$ & 0.00363 & 0.00382 & $-0.93\sigma$ & $\Dmark$ \\
$|V_{cd}|$ & 0.22397 & 0.22100 & $+0.74\sigma$ & $\Dmark$ \\
$|V_{cs}|$ & 0.97371 & 0.97500 & $-0.22\sigma$ & $\Dmark$ \\
$|V_{cb}|$ & 0.04166 & 0.04080 & $+0.61\sigma$ & $\Dmark$ \\
$|V_{td}|$ & 0.00842 & 0.00860 & $-0.90\sigma$ & $\Dmark$ \\
$|V_{ts}|$ & 0.04096 & 0.04150 & $-0.60\sigma$ & $\Dmark$ \\
$|V_{tb}|$ & 0.99913 & 1.01400 & $-0.51\sigma$ & $\Dmark$ \\
\midrule
$\delta_{\text{CKM}}$ & $64.3^\circ$ & $65.5 \pm 2.8^\circ$
  & $-0.43\sigma$ & $\Dmark$ \\
$J$ & $2.97 \times 10^{-5}$ & $3.08 \times 10^{-5}$
  & $-0.71\sigma$ & $\Dmark$ \\
\bottomrule
\end{tabular}
\end{table}
...
11 & $m_d$ & 4.681 & 4.67 & $+0.02\sigma$ & $\Dmark$ \\
12 & $m_s$ & 93.4 & 93.4 & $\sim 0$ & $\Dmark$ \\
13 & $m_b$ & 4180 & 4180 & $-0.01\sigma$ & $\Dmark$ \\
%
\midrule
\multicolumn{6}{l}{\textbf{Sector mass sums (MeV)}} \\
14 & $\Slep$ & 1882.8 & 1882.7 & $\sim 0$ & $\Dmark$ \\
15 & $\Sup$ & 174042 & 174030 & $\sim 0$ & $\Dmark$ \\
16 & $\Sdown$ & 4278 & 4278 & $\sim 0$ & $\Dmark$ \\
17 & $\Snu$ (meV) & 58.6 & $< 120$ & within & $\Dmark$ \\
%
\midrule
\multicolumn{6}{l}{\textbf{CKM matrix}} \\
18 & $|V_{ud}|$ & 0.97456 & 0.97373 & $+2.7\sigma$ & $\Dmark$ \\
19 & $|V_{us}|$ & 0.22409 & 0.2243 & $-0.42\sigma$ & $\Dmark$ \\
20 & $|V_{ub}|$ & 0.00363 & 0.00382 & $-0.93\sigma$ & $\Dmark$ \\
21 & $|V_{cd}|$ & 0.22396 & 0.221 & $+0.74\sigma$ & $\Dmark$ \\
22 & $|V_{cs}|$ & 0.97371 & 0.975 & $-0.22\sigma$ & $\Dmark$ \\
23 & $|V_{cb}|$ & 0.04166 & 0.0408 & $+0.61\sigma$ & $\Dmark$ \\
24 & $|V_{td}|$ & 0.00842 & 0.0086 & $-0.90\sigma$ & $\Dmark$ \\
25 & $|V_{ts}|$ & 0.04096 & 0.0415 & $-0.60\sigma$ & $\Dmark$ \\
26 & $|V_{tb}|$ & 0.99913 & 1.014 & $-0.51\sigma$ & $\Dmark$ \\
27 & $\delta_{\text{CKM}}$ & $64.3^\circ$ & $65.5 \pm 2.8^\circ$
  & $-0.44\sigma$ & $\Dmark$ \\
28 & $J$ (Jarlskog) & $2.97 \times 10^{-5}$ & $3.08 \times 10^{-5}$
  & $-0.71\sigma$ & $\Dmark$ \\
%
\midrule
...
\begin{itemize}
\item 32 agree within $1\sigma$ (78\%),
...
\item 3 exceed $2\sigma$: $m_e$ and $m_\mu$ (limited by the Koide
  ...
  (the Cabibbo angle anomaly at $2.7\sigma$).
\end{itemize}
...
\item \textbf{Down quark mass $m_d = 4.681$~MeV.}
...
\item \textbf{Down quarks:} The tree-level errors of 0.7--1.0\% are
  eliminated by the cross-representation vertex correction
  (Derivation~\ref{der:vertex_correction}): the factor
  $(952/893)^{3/20}$ reduces the sector sum error from $0.96\%$ to
  $0.008\%$.  Individual mass errors are now $\lesssim 0.02\sigma$.
