...
use crate::coupling::alpha::alpha_inverse;
use crate::coupling::alpha_s::{alpha_s_mz, m_gut_gev};
use crate::coupling::weinberg::{sin2_theta_w_gut, sin2_theta_w_mz};
use crate::higgs::mass::higgs_mass_default;
use crate::higgs::quartic::{higgs_quartic, second_scalar_mass};
...
pub fn compute_scorecard(digits: u32) -> Vec<Prediction> {
    ...
    results.push(Prediction::new(
        "m_u",
        Category::UpQuarkMass,
        "Koide(Σ_u, 10^{1/4}, 5⁴/6⁵)",
        m_u,
        Some(2.16),
        Some(0.49),
        "MeV",
    ...
    results.push(Prediction::new(
        "m_d",
        Category::DownQuarkMass,
        "Koide(Σ_d, (10-√2)^{1/4}, 1/6)",
        m_d,
        Some(4.67),
        Some(0.48),
        "MeV",
    ...
    let sigma_lep_exp = 0.51099895 + 105.6583755 + 1776.86; // 1883.029
    results.push(Prediction::new(
        "Σ_lep",
        Category::MassSum,
        "m_P × exp(-(9R+δ)/28)",
        sigma_lep,
        Some(sigma_lep_exp),
        Some(sigma_lep_exp * 0.0001), // 0.01% uncertainty from m_τ
        "MeV",
        Status::Derived,
    ));
    ...
    let sigma_up_exp = 2.16 + 1270.0 + 172760.0; // 174032.16
    results.push(Prediction::new(
        "Σ_up",
        Category::MassSum,
        "(3/4)m_P × exp(-(8R+δ)/28)",
        sigma_up,
        Some(sigma_up_exp),
        Some(sigma_up_exp * 0.002), // ~0.2% from m_t uncertainty
        "MeV",
        Status::Derived,
    ));
    ...
    let sigma_down_exp = 4.67 + 93.4 + 4180.0; // 4278.07
    results.push(Prediction::new(
        "Σ_down",
        Category::MassSum,
        "(9/4)m_P × exp(-(9R+δ)/28)",
        sigma_down,
        Some(sigma_down_exp),
        Some(sigma_down_exp * 0.01), // ~1% from QCD floor
        "MeV",
        Status::Derived,
    ));
    ...
    results.push(Prediction::new(
        "Σ_ν",
        Category::MassSum,
        "√(10/13)·m_P × exp(-(14R+δ)/28)",
        sigma_nu,
        None, // No direct measurement; KATRIN bound < 120 meV
        None,
        "meV",
        Status::Derived,
    ));
    ...
    // PDG 2024 CKM values (direct measurements)
    let ckm_data = [
        ("V_ud", 0.97373, 0.00031),  // superallowed 0+→0+
        ("V_us", 0.2243, 0.0005),    // K→πeν
        ("V_ub", 0.00382, 0.00020),  // B→πlν average
        ("V_cd", 0.221, 0.004),      // D→πlν
        ("V_cs", 0.975, 0.006),      // D→Klν
        ("V_cb", 0.0408, 0.0014),    // B→D(*)lν inclusive
        ("V_td", 0.0086, 0.0002),    // Bd mixing
        ("V_ts", 0.0415, 0.0009),    // Bs mixing
        ("V_tb", 1.014, 0.029),      // single top
    ];
    ...
    for (i, &(name, exp_val, exp_err)) in ckm_data.iter().enumerate() {
        results.push(Prediction::new(
            name,
            Category::CKM,
            "Fritzsch + octonionic CP",
            ckm.magnitudes[i].to_f64(),
            Some(exp_val),
            Some(exp_err),
            "",
            Status::Derived,
    ...
    // #28: Jarlskog invariant
    results.push(Prediction::new(
        "J (Jarlskog)",
        Category::CKM,
        "Im(V_ud V_cs V*_us V*_cd)",
        ckm.jarlskog.to_f64(),
        Some(3.08e-5),
        Some(0.15e-5),
        "",
    ...
    // ═══════════════════════════════════════════════════════════
    let nu1 = masses.nu1.to_f64(); // meV
    let nu2 = masses.nu2.to_f64();
    let nu3 = masses.nu3.to_f64();

    // #33-35: Individual neutrino masses (predictions, no experimental values)
    ...
    // #36: Δm²₂₁ (convert meV² → eV²: divide by 10⁶)
    ...
    results.push(Prediction::new(
        ...
        "m_H × √(7/12)",
        m_s_val,
        Some(95.4), // ATLAS/CMS diphoton excess at 95.4 GeV (3.1σ)
        Some(2.0),
        "GeV",
        Status::Derived,
    ));
