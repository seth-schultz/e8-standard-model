...
use crate::precision::{exp, mpf_u64, precision_bits};
...
pub fn sector_sum(a: u32, f: &Float) -> Float {
    ...
    Float::with_val(precision_bits(), Float::with_val(precision_bits(), f * &mp) * &boltzmann)
}

/// Compute sector sum with precise f = p/q rational factor.
pub fn sector_sum_rational(a: u32, f_num: u64, f_den: u64) -> Float {
    let f = Float::with_val(precision_bits(), f_num) / Float::with_val(precision_bits(), f_den);
    sector_sum(a, &f)
}

/// All four sector sums from the E8 mass formula.
pub struct SectorSums {
    pub leptons: Float,  // A = dim(su(3))+1 = 9, f = 1
    pub up: Float,       // A = dim(su(3)) = 8, f = 1/C₂(SU3,fund)
    pub down: Float,     // A = dim(su(3))+1 = 9, f = N_c/C₂(SU3,fund)
    pub neutrino: Float, // A = dim(G₂) = 14, f = √((|W(G₂)|-rank(G₂))/(|W(G₂)|+1))
}
...
pub fn compute_all_sector_sums() -> SectorSums {
    ...
    let f_up = Float::with_val(prec, c2_den) / Float::with_val(prec, c2_num);

    // f_down = N_c/C₂ = N_c × den/num = 3 × 3/4 = 9/4
    let f_down = Float::with_val(prec, N_C * c2_den) / Float::with_val(prec, c2_num);

    // f_ν = √((|W(G₂)| - rank(G₂)) / (|W(G₂)| + 1)) = √(10/13)
    let w_minus_r = G2.weyl_order - G2.rank as u64; // 12 - 2 = 10
    let w_plus_1 = G2.weyl_order + 1;                // 12 + 1 = 13
    let f_nu = (Float::with_val(prec, w_minus_r) / Float::with_val(prec, w_plus_1)).sqrt();
...
mod tests {
    ...
    fn test_lepton_sum() {
        ...
        let sigma = sector_sum_rational(9, 1, 1);
    ...
    fn test_up_sum() {
        crate::precision::set_precision(50);
        let sigma = sector_sum_rational(8, 3, 4);
        let val = sigma.to_f64();
    ...
    fn test_down_sum() {
        crate::precision::set_precision(50);
        let sigma = sector_sum_rational(9, 9, 4);
        let val = sigma.to_f64();
